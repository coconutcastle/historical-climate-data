...
export async function getAllStationMetadata(): Promise<StationMetadata[]> {
  return axios.get('http://0.0.0.0:9999/api/ghcnmv2/station', {
    headers: getHeaders
  }).then(res => res.data);
}

export async function getAllBasicStationMetadata(): Promise<StationMetadataBasic[]> {
  return axios.get('http://0.0.0.0:9999/api/ghcnmv2/basic/station', {
    headers: getHeaders
  }).then(res => res.data);
}

export async function getAllCountries(): Promise<CountryInfo[]> {
  return axios.get('http://0.0.0.0:9999/api/ghcnmv2/countries', {
    headers: getHeaders
  }).then(res => res.data);
}

export async function getAllRegions(): Promise<RawRegions[]> {
  return axios.get('http://0.0.0.0:9999/api/ghcnmv2/regions', {
    headers: getHeaders
  }).then(res => res.data);
}

export async function getDownloadData(byStation: boolean, dataParams: ParamsFields): Promise<any> {
  return axios.get('http://0.0.0.0:9999/api/ghcnmv2/download', {
    headers: getHeaders,
    params: {
      byStation,
      ...dataParams
